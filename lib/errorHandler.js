...
export function getTransactionErrorMessage(error) {
  ...
  // Bundler / AA simulation errors (eth_sendTransaction reports UNKNOWN_ERROR with UserOperation details)
  if (error.code === 'UNKNOWN_ERROR' && error.message?.includes('UserOperation reverted during simulation')) {
    // Detect common Solidity panic code 0x11 (arithmetic overflow/underflow)
    if (error.message.includes('0x4e487b71') && error.message.endsWith('00000011')) {
      return '⚠️ Simulation failed (panic 0x11). This typically happens when the network simulator disagrees on math during preprocessing. Try the following:\n• Re-approve to refresh allowance (interest accrued)\n• Retry with the exact owed amount (use Repay All)\n• If it persists, wait a few seconds and try again';
    }
    return '⚠️ Simulation failed on the network. Please retry in a few seconds or re-approve your allowance.';
  }
